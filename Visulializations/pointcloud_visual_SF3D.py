...
sys.path.append(os.path.join(os.environ['HOME'], "Documents/PycharmProjects/iccv_2019"))
...
file = "Area_6_office_32_7"
# file = "17DRP5sb8fy_0_10_"
...
depth_map = np.load(os.path.join(data_path_est, "depth_up_est/" + file + ".npy"))  # [:, :, 0]
depth_map_GT = np.load(os.path.join(data_path_gt, "depth_up/" + file + ".npy"))

mask = depth_map_GT > 0
depth_map[(mask * (-1) + 1).astype(np.bool)] = 0

# depth_map = depth_map_GT

rgb_map = cv2.imread(os.path.join(data_path_gt, "image_up/" + file + ".png"))
